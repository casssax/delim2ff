        ...
        while delm != -1:   #not last field in record
            ...
            if delm == 0:   #if empty field
                ...
                if (delm - 2) > abs(fields[count]):   #truncataes if field longer than layout (for header records)
                    ...
                    if fields[count] < 1:
                        new = new + line[1:delm - 1].replace("\"","").rjust(abs(fields[count]))   #right justify if all digits
                    else:
                        new = new + line[1:delm - 1].replace("\"","").ljust(fields[count])
                count = count + 1
                ...
                if pos > abs(fields[count]):   #truncataes if field longer than layout (for header records)
                    new = new + line[1:abs(fields[count]) + 1].replace("\"","")   
                elif fields[count] < 1:
                    new = new + (line[1:-2].replace("\"","").rjust(abs(fields[count])))   #right justify if all digits
                else:
                    new = new + (line[1:-2].replace("\"","").ljust(fields[count]))
                out_file.write(new + '\n')
    out_file.close()
    file.close()
...
def fix_field(file):
    ...
    for line in file:
        ...
        pos = 0
        while pos != -1:
            pos = line.find(delim_type)
            if pos != -1:   #if not last field in record
                if pos > abs(fields[count]):
                    new = new + line[0:abs(fields[count])]   #truncataes if field longer than layout (for header records)
                else:
                    if fields[count] < 1:
                        new = new + line[0:pos].rjust(abs(fields[count]))   #right justify if all digits
                    else:
                        new = new + line[0:pos].ljust(fields[count])
                count = count + 1
                line = line[pos + 1:]   #increments field number
            else:
                if pos > abs(fields[count]):
                    new = new + line[0:abs(fields[count])]   #truncataes if field longer than layout (for header records)
                elif fields[count] < 1:
                    new = new + (line[0:-1].rjust(abs(fields[count])))   #right justify if all digits
                else:
                    new = new + (line[0:-1].ljust(fields[count]))
                out_file.write(new + '\n')
    out_file.close()
    file.close()
    print('total records processed: {}'.format(total))
...
out_file_path = script_path + '\\' + outname(file_name) + '.ff'
